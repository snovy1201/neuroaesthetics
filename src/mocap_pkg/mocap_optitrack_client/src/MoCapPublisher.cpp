...
void MoCapPublisher::sendRigidBodyMessage(double cameraMidExposureSecsSinceEpoch, sRigidBodyData* bodies_ptr, int nRigidBodies)
{
  ...
  for(int i=0; i < nRigidBodies; i++)
  {
    ...
    double tipDistance;
    if (i == 0)
      this->get_parameter("tipDistance1", tipDistance);
    else if (i == 1)
      this->get_parameter("tipDistance2", tipDistance);
    else
      this->get_parameter("tipDistance3", tipDistance);

    //Direction of the orientation is kinda arbitrary, so we calculate both a neg and pos normal vector
    Eigen::Vector3d localTip(0,tipDistance, 0);
    Eigen::Vector3d tipWorldUp = currPos + currQuat * localTip;
    Eigen::Vector3d tipWorldDown = currPos - currQuat * localTip;

    Eigen::Vector3d tipWorld = (tipWorldUp.y() <= tipWorldDown.y()) ? tipWorldUp : tipWorldDown;

    bool isTouching = ((tipWorld.y() <= tolerance)) && ((tipWorld.y() > -ignore));

    RCLCPP_INFO(get_logger(), "passed");


    RCLCPP_INFO(this->get_logger(), "Rigid Body [ID=%d  Error=%3.2f  Valid=%d]\n", bodies[i].ID, bodies[i].MeanError, bodies[i].params & 0x01);
    RCLCPP_INFO(this->get_logger(), "\tx\ty\tz\tqx\tqy\tqz\tqw\tisStroke\tProximity\n");
    RCLCPP_INFO(this->get_logger(), "\t%3.5f\t%3.5f\t%3.5f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%d\t%3.2f\n",
      bodies[i].x,
      bodies[i].y,
      bodies[i].z,
      bodies[i].qx,
      ...
      rb.is_stroke = isTouching;
      rb.proximity = tipWorld.y();
      //
